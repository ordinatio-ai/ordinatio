...
import { buildTrialReport, createTrialArtifact } from '@ordinatio/core';
...
function collectTests(testModule: TestModule): PugilTestResult[] {
  ...
  for (const testCase of testModule.children.allTests()) {
    ...
    results.push({ name: tc.fullName, file: filename, passed, durationMs: diag?.duration ?? 0, ...(error ? { error } : {}) });
  }
  return results;
}
...
export default class PugilReporter implements Reporter {
  onTestRunEnd(testModules: ReadonlyArray<TestModule>) {
    ...
    for (const mod of testModules) allTests.push(...collectTests(mod));
    const passed = allTests.filter((t) => t.passed).length;
    const failed = allTests.filter((t) => !t.passed).length;
    const totalDuration = allTests.reduce((sum, t) => sum + t.durationMs, 0);
    const report = buildTrialReport({ subject: PUGIL_SUBJECT, totalTests: allTests.length, passed, failed, durationMs: totalDuration, tests: allTests, categoryMap: PUGIL_CATEGORY_MAP });
    const cycleId = process.env.PUGIL_CYCLE_ID ?? `standalone-${Date.now()}`;
    ...
    console.log(`\n[Pugil] Trial report: ${report.assessment} — ${passed}/${allTests.length} tests passed`);
