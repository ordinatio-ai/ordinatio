...
async function executeNodes(
  ...
  while (queue.length > 0) {
    ...
    try {
      ...
      enqueueNextNodes(nextEdgeTypes, outEdges.get(nodeId) || [], queue);
    } catch (error) {
      handleExecutionError(nodeId, nodeState, state, error);
    }
  }
}

function enqueueNextNodes(nextEdgeTypes: DagEdgeType[], edges: DagEdge[], queue: string[]) {
  for (const edge of edges) {
    if (nextEdgeTypes.includes(edge.type)) {
      queue.push(edge.targetNodeId);
    }
  }
}

function handleExecutionError(nodeId: string, nodeState: DagNodeState, state: DagExecutionState, error: any) {
  nodeState.status = 'failed';
  addLog(state, nodeId, 'error', `Node execution failed: ${error.message}`);
  classifyFailure(nodeState, error);
}
